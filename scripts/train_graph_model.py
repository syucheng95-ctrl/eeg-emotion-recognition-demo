...
from train_gnn import CHANNELS, HOLDOUT_DEFAULT, build_adjacency, build_edge_pairs


def build_edge_index() -> torch.Tensor:
    channel_to_idx = {name: idx for idx, name in enumerate(CHANNELS)}
    edges = []
    for left, right in build_edge_pairs():
        i, j = channel_to_idx[left], channel_to_idx[right]
        edges.append((i, j))
        edges.append((j, i))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train graph model with subject-level validation.")
    ...
    parser.add_argument("--conv-type", type=str, default="gcn", choices=["gcn", "cheb", "gat", "adaptive_gcn"])
    ...
    parser.add_argument("--report-name", type=str, default="graph_model_report.json")
    parser.add_argument("--model-name", type=str, default="graph_model.pt")
...
def train_one_split(x_train, y_train, x_valid, y_valid, args, device, base_adj, edge_index):
    ...
    if args.standardize_input:
        mean, std = fit_window_normalizer(x_train)
        x_train = apply_window_normalizer(x_train, mean, std)
...
def main() -> None:
    ...
    final_model = GraphSequenceClassifier(
        conv_type=args.conv_type,
        temporal_type=args.temporal_type,
    ...
    if final_stats is not None:
        model_payload["input_mean"] = final_stats["mean"]
        model_payload["input_std"] = final_stats["std"]
